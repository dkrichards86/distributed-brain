...
Physical storage units in [[ClickHouse Architecture|ClickHouse]] — each INSERT creates a new immutable directory on disk containing one file per column, a primary index, and any secondary indexes; background merging consolidates them over time.
...
- `primary.idx` — the sparse primary key index (one mark per [[ClickHouse Granules|granule]])
...
1. **INSERT** lands as a new part with a name encoding the [[ClickHouse Partitions|partition]] key and a sequence number (e.g., `20240101_1_1_0`).
