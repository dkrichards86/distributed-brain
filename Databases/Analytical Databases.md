...
Typically use [[Columnar Databases|columnar storage]] internally to minimize I/O on wide tables. Queries are compiled and parallelized across many nodes. Pre-computed materialized views and aggregations speed up common query patterns. Data is usually loaded in bulk (ETL/ELT pipelines) rather than inserted row by row. Query planners are tuned for full-table scans and hash joins rather than index lookups.
