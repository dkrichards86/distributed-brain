...
After saving a new note, scan all existing notes for concepts that relate to the new note and add `[[ ]]` backlinks in both directions — in the new note and in any existing notes that reference the same concept. Also look for backlink opportunities whenever editing existing notes.
